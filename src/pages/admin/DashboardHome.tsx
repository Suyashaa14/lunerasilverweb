...
interface NetProfitPoint {
  month: string;
  netProfit: number;
}

// One payload for the whole page; the server reads each table once and returns
// every aggregate the dashboard draws.
interface Overview {
  stats: Stats;
  sales: SalesSummary;
  expenses: ExpensesSummary;
  expensesByCategory: ExpensesCategoryPoint[];
  netProfitTrend: NetProfitPoint[];
  netProfit: number;
}
...
export function DashboardHome() {
  ...
  const allTimeSales = overview.sales;
  const allTimeExpenses = overview.expenses;
  const categoryBreakdown = overview.expensesByCategory;
  const trend = overview.netProfitTrend;
  ...
  const allTimeNetProfit = overview.netProfit;
    ...
    <div>
      ...
      <div className="bg-white border border-neutral-200 rounded-lg p-5 mb-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          ...
          <StatTile
            label="Net profit"
            value={`Rs ${allTimeNetProfit.toLocaleString()}`}
            tone={allTimeNetProfit >= 0 ? 'good' : 'bad'}
          />
        </div>
      </div>
